// $Id: OS_NS_sys_utsname.cpp,v 1.2 2009/03/19 05:29:23 qinghua Exp $

#include "os/OS_NS_sys_utsname.h"

ACE_RCSID(ace, OS_NS_sys_utsname, "$Id: OS_NS_sys_utsname.cpp,v 1.2 2009/03/19 05:29:23 qinghua Exp $")
...
int
OS::uname (ACE_utsname *name)
{
  OS_TRACE ("OS::uname");
#if !defined (ACE_LACKS_UNAME)
  OSCALL_RETURN (::uname (name), int, -1);
#elif defined (ACE_WIN32)
  size_t maxnamelen = sizeof name->nodename;
  OS::strcpy (name->sysname, "Win32");
...
#   endif
      ...
      switch (arch)
        ...
        case PROCESSOR_ARCHITECTURE_INTEL:
          OS::strcpy (processor, "Intel");
          if (sinfo.wProcessorLevel == 3)
            OS::strcpy (subtype, "80386");
          else if (sinfo.wProcessorLevel == 4)
            OS::strcpy (subtype, "80486");
          else if (sinfo.wProcessorLevel == 5)
            OS::strcpy (subtype, "Pentium");
          else if (sinfo.wProcessorLevel == 6)
            OS::strcpy (subtype, "Pentium Pro");
          else if (sinfo.wProcessorLevel == 7)  // I'm guessing here
            OS::strcpy (subtype, "Pentium II");
          break;
        case PROCESSOR_ARCHITECTURE_MIPS:
          OS::strcpy (processor, "MIPS");
          OS::strcpy (subtype, "R4000");
          break;
        case PROCESSOR_ARCHITECTURE_ALPHA:
          OS::strcpy (processor, "Alpha");
          OS::sprintf (subtype, "%d", sinfo.wProcessorLevel);
          break;
        case PROCESSOR_ARCHITECTURE_PPC:
          OS::strcpy (processor, "PPC");
          if (sinfo.wProcessorLevel == 1)
            OS::strcpy (subtype, "601");
          else if (sinfo.wProcessorLevel == 3)
            OS::strcpy (subtype, "603");
          else if (sinfo.wProcessorLevel == 4)
            OS::strcpy (subtype, "604");
          else if (sinfo.wProcessorLevel == 6)
            OS::strcpy (subtype, "603+");
          else if (sinfo.wProcessorLevel == 9)
            OS::strcpy (subtype, "804+");
          else if (sinfo.wProcessorLevel == 20)
            OS::strcpy (subtype, "620");
          break;
#     if defined PROCESSOR_ARCHITECTURE_IA64
        case PROCESSOR_ARCHITECTURE_IA64:
          OS::strcpy (processor, "Itanium");
          OS::sprintf (subtype, "%d",
                           sinfo.wProcessorLevel);
          break;
#     endif
#     if defined PROCESSOR_ARCHITECTURE_AMD64
        case PROCESSOR_ARCHITECTURE_AMD64:
          OS::strcpy (processor, "x64");
          OS::sprintf (subtype, "%d",
                           sinfo.wProcessorLevel);
          break;
#     endif
...
#     endif
        ...
        default:
          ...
          OS::strcpy (processor, "Unknown");
          break;
        }
      OS::sprintf (name->machine,
                       "%s %s",
                       processor, subtype);
    }
  else if (vinfo.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS)
    {
      if (vinfo.dwMajorVersion == 4 && vinfo.dwMinorVersion == 0)
        {
          OS::strcpy (name->release, "Windows 95");
          if (vinfo.szCSDVersion[1] == ACE_TEXT('C'))
            OS::strcat (name->release, " OSR2");
        }
      else if (vinfo.dwMajorVersion == 4 && vinfo.dwMinorVersion == 10)
        {
          OS::strcpy (name->release, "Windows 98");
          if (vinfo.szCSDVersion[1] == ACE_TEXT('A'))
            OS::strcat (name->release, " SE");
        }
      else if (vinfo.dwMajorVersion == 4 && vinfo.dwMinorVersion == 90)
        {
          OS::strcpy (name->release, "Windows Me");
        }
      else
        {
          OS::strcpy (name->release, unknown);
        }

      OS::sprintf (name->version, "%d", LOWORD (vinfo.dwBuildNumber));
      if (sinfo.dwProcessorType == PROCESSOR_INTEL_386)
        OS::strcpy (name->machine, "Intel 80386");
      else if (sinfo.dwProcessorType == PROCESSOR_INTEL_486)
        OS::strcpy (name->machine, "Intel 80486");
      else if (sinfo.dwProcessorType == PROCESSOR_INTEL_PENTIUM)
        OS::strcpy (name->machine, "Intel Pentium");
      else
        OS::strcpy (name->machine, unknown);
    }
  else
    {
      ...
      OS::strcpy (name->release, unknown);
      OS::strcpy (name->version, unknown);
      OS::strcpy (name->machine, unknown);
    }
...
# else /* ACE_LACKS_HOSTNAME */
  return OS::hostname (name->nodename, maxnamelen);
# endif /* ACE_LACKS_HOSTNAME */

#elif defined (ACE_VXWORKS)
  size_t maxnamelen = sizeof name->nodename;
  OS::strcpy (name->sysname, "VxWorks");
  OS::strcpy (name->release, "???");
  OS::strcpy (name->version, sysBspRev ());
  OS::strcpy (name->machine, sysModel ());
