 ...
 *  @file   OS_NS_time.h
 *
 *  $Id: OS_NS_time.h,v 1.2 2009/03/19 05:29:23 qinghua Exp $
 *
...
/// Helper for the OS::timezone() function
/**
 * We put all the timezone stuff that used to be in OS::timezone()
 * here because on some platforms "timezone" is a macro.  Because of this,
 * the name OS::timezone will cause errors.  So in order to use the
 * macro as it is defined but also keep the name OS::timezone, we
...
inline long ace_timezone()
{
  ...
  OSCALL (::gettimeofday (&time, &zone), int, -1, result);
...
/// Helper for the OS::difftime() function
/**
 * We moved the difftime code that used to be in OS::difftime()
 * here because on some platforms "difftime" is a macro.  Because of this,
 * the name OS::difftime will cause errors.  So in order to use the
 * macro as it is defined but also keep the name OS::difftime, we
