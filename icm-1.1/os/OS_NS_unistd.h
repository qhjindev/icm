 ...
 *  @file   OS_NS_unistd.h
 *
 *  $Id: OS_NS_unistd.h,v 1.2 2009/03/19 05:29:23 qinghua Exp $
 *
...
#ifndef OS_NS_UNISTD_H
# define OS_NS_UNISTD_H
...
namespace OS
{
  ...
  // used by ARGV::argv_to_string() and OS::fork_exec()
  extern ACE_Export
   ...
   * <OS::read> call, which uses the <read> system call on UNIX
  ...
  ACE_NAMESPACE_INLINE_FUNCTION
  useconds_t ualarm (const TimeValue &tv,
                     const TimeValue &tv_interval = TimeValue::zero);
  ...
  /**
   * Send @a len bytes from @a buf to <handle> (uses the <OS::write>
