// $Id: OS_NS_stdio.cpp,v 1.2 2009/03/19 05:29:23 qinghua Exp $
...
ACE_INLINE void
OS::flock_adjust_params (OS::ace_flock_t *lock,
                             short whence,
                             ACE_OFF_T &start,
                             ACE_OFF_T &len)
{
...
# endif /* _FILE_OFFSET_BITS == 64 */
    ...
    case SEEK_END:
      {
        ACE_OFF_T const size = OS::filesize (lock->handle_);
        if (size == -1)
          return;
  ...
  if (len == 0)
    {
      ACE_OFF_T const tlen = OS::filesize (lock->handle_);
      if (tlen != -1)
        len = tlen - start;
    }
}
...
ACE_INLINE int
OS::flock_init (OS::ace_flock_t *lock,
                    int flags,
                    const ACE_TCHAR *name,
                    mode_t perms)
{
  OS_TRACE ("OS::flock_init");
#if defined (ACE_WIN32)
  ...
  if (name != 0)
    {
      OSCALL (OS::open (name, flags, perms),
                  ACE_HANDLE,
                  ACE_INVALID_HANDLE,
                  lock->handle_);
      if (lock->handle_ != ACE_INVALID_HANDLE)
        lock->lockname_ = OS::strdup (name);
...
ACE_INLINE int
OS::flock_unlock (OS::ace_flock_t *lock,
                      short whence,
                      ACE_OFF_T start,
                      ACE_OFF_T len)
{
  OS_TRACE ("OS::flock_unlock");
  ...
  OS::flock_adjust_params (lock, whence, start, len);
  ...
  // release lock
  OSCALL_RETURN (OS::fcntl (lock->handle_, F_SETLK,
...
ACE_INLINE int
OS::flock_destroy (OS::ace_flock_t *lock,
                       int unlink_file)
{
  OS_TRACE ("OS::flock_destroy");
  if (lock->handle_ != ACE_INVALID_HANDLE)
    {
      OS::flock_unlock (lock);
      // Close the handle.
      OS::close (lock->handle_);
      lock->handle_ = ACE_INVALID_HANDLE;
      if (lock->lockname_ != 0)
        {
          if (unlink_file)
            OS::unlink (lock->lockname_);
          OS::free (
...
ACE_INLINE int
OS::flock_rdlock (OS::ace_flock_t *lock,
                      short whence,
                      ACE_OFF_T start,
                      ACE_OFF_T len)
{
  OS_TRACE ("OS::flock_rdlock");
#if defined (ACE_LACKS_FILELOCKS)
  ACE_UNUSED_ARG (lock);
  ACE_UNUSED_ARG (whence);
  ACE_UNUSED_ARG (start);
...
#  endif /* ACE_HAS_WINNT4 && (ACE_HAS_WINNT4 != 0) */
  ...
  // block, if no access
  OSCALL_RETURN (OS::fcntl (lock->handle_, F_SETLKW,
                                    reinterpret_cast<long> (&lock->lock_)),
                     int, -1);
#endif /* ACE_WIN32 */
}
...
ACE_INLINE int
OS::flock_tryrdlock (OS::ace_flock_t *lock,
                         short whence,
                         ACE_OFF_T start,
                         ACE_OFF_T len)
{
  OS_TRACE ("OS::ace_flock_tryrdlock");
#if defined (ACE_LACKS_FILELOCKS)
  ACE_UNUSED_ARG (lock);
  ACE_UNUSED_ARG (whence);
  ACE_UNUSED_ARG (start);
...
#  endif /* ACE_HAS_WINNT4 && (ACE_HAS_WINNT4 != 0) */
  ...
  OSCALL (OS::fcntl (lock->handle_, F_SETLK,
                             reinterpret_cast<long> (&lock->lock_)),
              int, -1, result);
...
ACE_INLINE int
OS::flock_trywrlock (OS::ace_flock_t *lock,
                         short whence,
                         ACE_OFF_T start,
                         ACE_OFF_T len)
{
  OS_TRACE ("OS::ace_flock_trywrlock");
#if defined (ACE_LACKS_FILELOCKS)
  ACE_UNUSED_ARG (lock);
  ACE_UNUSED_ARG (whence);
  ACE_UNUSED_ARG (start);
...
ACE_INLINE int
OS::flock_wrlock (OS::ace_flock_t *lock,
                      short whence,
                      ACE_OFF_T start,
                      ACE_OFF_T len)
{
  OS_TRACE ("OS::flock_wrlock");
#if defined (ACE_LACKS_FILELOCKS)
  ACE_UNUSED_ARG (lock);
  ACE_UNUSED_ARG (whence);
  ACE_UNUSED_ARG (start);
...
ACE_INLINE char *
OS::cuserid (char *user, size_t maxlen)
{
  OS_TRACE ("OS::cuserid");
...
#  endif
  ...
  ::setpwent ();
  // Should use OS::setpwent() but I didn't want to move this
  // method after it.
  ...
  if (OS::strlen (pw->pw_name) <= max_length)
    {
      return OS::strcpy (userid, pw->pw_name);
    }
  else
    {
  ...
  ACE_UNUSED_ARG (maxlen);
  OSCALL_RETURN (::ace_cuserid(user), char*, 0);
#endif
}

#if defined (ACE_HAS_WCHAR)
ACE_INLINE wchar_t *
OS::cuserid (wchar_t *user, size_t maxlen)
{
...
# else
  ...
  if (OS::cuserid (char_user, maxlen))
    {
      OS::strcpy (user, ACE_Ascii_To_Wide (char_user).wchar_rep ());
      result = user;
    }
...
{
  OS_TRACE ("OS::fclose");
  OSCALL_RETURN (ACE_STD_NAMESPACE::fclose (fp), int, -1);
}

ACE_INLINE FILE *
OS::fdopen (ACE_HANDLE handle, const ACE_TCHAR *mode)
{
  OS_TRACE ("OS::fdopen");
#if defined (ACE_HAS_WINCE)
  OSCALL_RETURN (::_wfdopen (handle, ACE_TEXT_ALWAYS_WCHAR (mode)),
                     FILE*,
                     0);
#elif defined (ACE_WIN32)
...
OS::fflush (FILE *fp)
{
  ...
  OSCALL_RETURN (ACE_STD_NAMESPACE::fflush (fp), int, -1);
}
...
ACE_INLINE int
OS::fgetpos (FILE* fp, fpos_t* pos)
{
  OSCALL_RETURN (ACE_STD_NAMESPACE::fgetpos (fp, pos), int, -1);
}

ACE_INLINE char *
OS::fgets (char *buf, int size, FILE *fp)
{
  OS_TRACE ("OS::fgets");
  OSCALL_RETURN (ACE_STD_NAMESPACE::fgets (buf, size, fp), char *, 0);
}

#if defined (ACE_HAS_WCHAR) && !defined(ACE_LACKS_FGETWS)
ACE_INLINE wchar_t *
OS::fgets (wchar_t *buf, int size, FILE *fp)
{
  OS_TRACE ("OS::fgets");
  OSCALL_RETURN (ACE_STD_NAMESPACE::fgetws (buf, size, fp), wchar_t *, 0);
}
...
ACE_INLINE FILE *
OS::fopen (const char *filename, const ACE_TCHAR *mode)
{
  OS_TRACE ("OS::fopen");
  OSCALL_RETURN
...
#if defined (ACE_HAS_WCHAR)
ACE_INLINE FILE *
OS::fopen (const wchar_t *filename, const ACE_TCHAR *mode)
{
  OS_TRACE ("OS::fopen");
#if defined (ACE_HAS_WINCE)
  OSCALL_RETURN
...
ACE_INLINE int
OS::ungetc (int c, FILE *fp)
{
  return ace_ungetc_helper (c, fp);
}

ACE_INLINE int
OS::fputc (int c, FILE *fp)
{
  return ace_fputc_helper (c, fp);
}

ACE_INLINE int
OS::putc (int c, FILE *fp)
{
  return ace_putc_helper (c, fp);
}

ACE_INLINE int
OS::fputs (const char *s, FILE *stream)
{
  OS_TRACE ("OS::fputs");
  OSCALL_RETURN (ACE_STD_NAMESPACE::fputs (s, stream), int, -1);
}

#if defined (ACE_HAS_WCHAR) && !defined(ACE_LACKS_FPUTWS)
ACE_INLINE int
OS::fputs (const wchar_t *s, FILE *stream)
{
  OS_TRACE ("OS::fputs");
  OSCALL_RETURN (ACE_STD_NAMESPACE::fputws (s, stream), int, -1);
}
#endif /* ACE_HAS_WCHAR && !ACE_LACKS_FPUTWS */

ACE_INLINE size_t
OS::fread (void *ptr, size_t size, size_t nelems, FILE *fp)
{
  OS_TRACE ("OS::fread");
  OSCALL_RETURN (ACE_STD_NAMESPACE::fread (ptr, size, nelems, fp),
                     size_t,
                     0);
}

ACE_INLINE FILE *
OS::freopen (const ACE_TCHAR *filename, const ACE_TCHAR *mode, FILE* stream)
{
  OS_TRACE ("OS::freopen");
#if defined (ACE_WIN32) && (defined(ACE_USES_WCHAR) || defined(ACE_HAS_WINCE))
  OSCALL_RETURN (::_wfreopen (ACE_TEXT_ALWAYS_WCHAR (filename),
...
ACE_INLINE int
OS::fseek (FILE *fp, long offset, int whence)
{
# if defined (ACE_WIN32)
...
# endif   /* ACE_WIN32 */
  OSCALL_RETURN (ACE_STD_NAMESPACE::fseek (fp, offset, whence), int, -1);
}

ACE_INLINE int
OS::fsetpos (FILE* fp, fpos_t* pos)
{
  OSCALL_RETURN (::fsetpos (fp, pos), int, -1);
}

ACE_INLINE long
OS::ftell (FILE* fp)
{
  OSCALL_RETURN (ACE_STD_NAMESPACE::ftell (fp), long, -1);
}

ACE_INLINE size_t
OS::fwrite (const void *ptr, size_t size, size_t nitems, FILE *fp)
{
  OS_TRACE ("OS::fwrite");
  OSCALL_RETURN (ACE_STD_NAMESPACE::fwrite (ptr, size, nitems, fp),
                     size_t,
                     0);
}

ACE_INLINE void
OS::perror (const char *s)
{
  OS_TRACE ("OS::perror");
...
ACE_INLINE int
OS::puts (const char *s)
{
  OS_TRACE ("OS::puts");
  OSCALL_RETURN (::puts (s), int, -1);
}

#if defined (ACE_HAS_WCHAR)
ACE_INLINE int
OS::puts (const wchar_t *s)
{
  OS_TRACE ("OS::puts");
#if defined (ACE_WIN32)
  OSCALL_RETURN (::_putws (s), int, -1);
#else /* ACE_WIN32 */
  // There's no putws()...
  ACE_Wide_To_Ascii n_s (s);
  OSCALL_RETURN (::puts (n_s.char_rep ()), int, -1);
#endif /* ACE_WIN32 */
}
#endif /* ACE_HAS_WCHAR */

ACE_INLINE int
OS::rename (const char *old_name,
                const char *new_name,
                int flags)
{
...
# else /* ACE_LACKS_RENAME */
  ACE_UNUSED_ARG (flags);
  OSCALL_RETURN (::rename (old_name, new_name), int, -1);
# endif /* ACE_LACKS_RENAME */
}

#if defined (ACE_HAS_WCHAR)
ACE_INLINE int
OS::rename (const wchar_t *old_name,
...
# elif defined (ACE_WIN32)
  ACE_UNUSED_ARG (flags);
  OSCALL_RETURN (::_wrename (old_name, new_name), int, -1);
# else /* ACE_LACKS_RENAME */
  ACE_Wide_To_Ascii nold_name (old_name);
  ACE_Wide_To_Ascii nnew_name (new_name);
  return OS::rename (nold_name.char_rep (), nnew_name.char_rep (), flags);
# endif /* ACE_LACKS_RENAME */
}
#endif /* ACE_HAS_WCHAR */
...
ACE_INLINE char *
OS::tempnam (const char *dir, const char *pfx)
{
  OS_TRACE ("OS::tempnam");
  ...
  OSCALL_RETURN (ACE_STD_NAMESPACE::tempnam (const_cast <char *> (dir), const_cast<char *> (pfx)), char *, 0);
#else /* ACE_LACKS_TEMPNAM */
  //OSCALL_RETURN (ACE_STD_NAMESPACE::tempnam (dir, pfx), char *, 0);
  return 0;
...
#if defined (ACE_HAS_WCHAR)
ACE_INLINE wchar_t *
OS::tempnam (const wchar_t *dir, const wchar_t *pfx)
{
  OS_TRACE ("OS::tempnam");
#if defined (ACE_LACKS_TEMPNAM)
  ACE_UNUSED_ARG (dir);
...
#  if defined (ACE_HAS_NONCONST_TEMPNAM)
  OSCALL_RETURN (::_wtempnam (const_cast <wchar_t*> (dir), const_cast <wchar_t*> (pfx)), wchar_t *, 0);
#  else
  OSCALL_RETURN (::_wtempnam (dir, pfx), wchar_t *, 0);
#  endif /* __BORLANDC__ */
#else /* ACE_LACKS_TEMPNAM */
  ...
  char *name = OS::tempnam (ndir, npfx);
  // OS::tempnam returns a pointer to a malloc()-allocated space.
  // Convert that string to wide-char and free() the original.
  wchar_t *wname = 0;
  if (name != 0)
    {
      size_t namelen = OS::strlen (name) + 1;
      wname = reinterpret_cast<wchar_t *>
        (OS::malloc (namelen * sizeof (wchar_t)));
      if (wname != 0)
        OS::strcpy (wname, ACE_Ascii_To_Wide (name).wchar_rep ());
      OS::free (name);
    }
  return wname;
#endif /* ACE_LACKS_TEMPNAM */
...
ACE_INLINE int
OS::vsprintf (char *buffer, const char *format, va_list argptr)
{
  return ::vsprintf (buffer, format, argptr);
}

ACE_INLINE int
OS::vsnprintf (char *buffer, size_t maxlen, const char *format, va_list ap)
{
...
#if defined (ACE_HAS_WCHAR)
ACE_INLINE int
OS::vsprintf (wchar_t *buffer, const wchar_t *format, va_list argptr)
{
...
# elif defined (ACE_WIN32)
  ...
  // ISO C standard. Also see OS::snprintf() for more info on this.
...
#if defined (ACE_HAS_WCHAR)
ACE_INLINE int
OS::vsnprintf (wchar_t *buffer, size_t maxlen, const wchar_t *format, va_list ap)
{
# if (defined _XOPEN_SOURCE && (_XOPEN_SOURCE - 0) >= 500) || \
     (defined (sun) && !(defined(_XOPEN_SOURCE) && (_XOPEN_VERSION-0==4))) || \
     (defined (ACE_HAS_DINKUM_STL) || defined (__DMC__)) || \
...
OS::get_win32_versioninfo ()
{
  return OS::win32_versioninfo_;
}

ACE_INLINE HINSTANCE
OS::get_win32_resource_module ()
{
  return OS::win32_resource_module_;
}

ACE_INLINE void
OS::set_win32_resource_module (HINSTANCE instance)
{
  OS::win32_resource_module_ = instance;
}

ACE_INLINE LPSECURITY_ATTRIBUTES
OS::default_win32_security_attributes (LPSECURITY_ATTRIBUTES sa)
{
...
ACE_TEXT_OSVERSIONINFO OS::win32_versioninfo_;
HINSTANCE OS::win32_resource_module_;
...
#     endif /* ACE_DISABLES_THREAD_LIBRARY_CALLS */
      OS::set_win32_resource_module(instance);
    }
...
void
OS::ace_flock_t::dump (void) const
{
#if defined (ACE_HAS_DUMP)
  OS_TRACE ("OS::ace_flock_t::dump");
...
void OS::checkUnicodeFormat (FILE* fp)
{
  if (fp != 0)
    {
      ...
      size_t numRead =
        OS::fread(&first_two_bytes, sizeof (first_two_bytes), 1, fp);
...
#if defined (ACE_WIN32)
              OS::fseek(fp, 0, FILE_BEGIN);
#else
              OS::fseek(fp, 0, SEEK_SET);
#endif /* ACE_WIN32 */
            }
        }
               ...
               const ACE_TCHAR *mode)
{
  ...
  ACE_HANDLE const handle = OS::open (filename, hmode);
...
#if defined (ACE_HAS_WCHAR)
FILE *
OS::fopen (const wchar_t *filename,
               const ACE_TCHAR *mode)
{
  OS_TRACE ("OS::fopen");
  int hmode = _O_TEXT;
  ...
  ACE_HANDLE handle = OS::open (filename, hmode);
  if (handle != ACE_INVALID_HANDLE)
    {
      hmode &= _O_TEXT | _O_RDONLY | _O_APPEND;
...
int
OS::fprintf (FILE *fp, const char *format, ...)
{
  OS_TRACE ("OS::fprintf");
  int result = 0;
  va_list ap;
  va_start (ap, format);
  OSCALL (::vfprintf (fp, format, ap), int, -1, result);
  va_end (ap);
  return result;
}
...
#if defined (ACE_HAS_WCHAR)
int
OS::fprintf (FILE *fp, const wchar_t *format, ...)
{
  OS_TRACE ("OS::fprintf");
...
# else
  ...
  va_list ap;
  va_start (ap, format);
  OSCALL (ACE_STD_NAMESPACE::vfwprintf (fp, format, ap), int, -1, result);
  va_end (ap);
  return result;
...
int
OS::printf (const char *format, ...)
{
  // OS_TRACE ("OS::printf");
  int result;
  va_list ap;
  va_start (ap, format);
  OSCALL (::vprintf (format, ap), int, -1, result);
  va_end (ap);
  return result;
}

int
OS::snprintf (char *buf, size_t maxlen, const char *format, ...)
{
  // OS_TRACE ("OS::snprintf");
  int result;
  va_list ap;
  va_start (ap, format);
  result = OS::vsnprintf (buf, maxlen, format, ap);
  va_end (ap);
  return result;
}
...
int
OS::snprintf (wchar_t *buf, size_t maxlen, const wchar_t *format, ...)
{
  // OS_TRACE ("OS::snprintf");
# if (defined _XOPEN_SOURCE && (_XOPEN_SOURCE - 0) >= 500) || \
...
#  elif defined (ACE_WIN32)
  ...
  OSCALL (::_vsnwprintf (buf, maxlen, format, ap), int, -1, result);
  // Win32 doesn't regard a full buffer with no 0-terminate as an overrun.
  if (result == static_cast <int> (maxlen))
    result = -1;
  ...
  if (result == -1)
    buf[maxlen-1] = '\0';
#  else
  OSCALL (::vswprintf (buf, maxlen, format, ap), int, -1, result);
#  endif /* ACE_WIN32 */
  va_end (ap);
  // In out-of-range conditions, C99 defines vsnprintf to return the number
  // of characters that would have been written if enough space was available.
...
int
OS::sprintf (char *buf, const char *format, ...)
{
  // OS_TRACE ("OS::sprintf");
  ...
  va_list ap;
  va_start (ap, format);
  OSCALL (::vsprintf (buf, format, ap), int, -1, result);
  va_end (ap);
  return result;
}
...
#if defined (ACE_HAS_WCHAR)
int
OS::sprintf (wchar_t *buf, const wchar_t *format, ...)
{
  OS_TRACE ("OS::sprintf");
...
# if (defined _XOPEN_SOURCE && (_XOPEN_SOURCE - 0) >= 500) || \
  ...
  va_list ap;
  va_start (ap, format);
  OSCALL (ACE_STD_NAMESPACE::vswprintf (buf, 4096, format, ap), int, -1, result);
  va_end (ap);
  return result;

# elif defined (ACE_WIN32)
  // Pre-VC8 Windows has vswprintf, but the signature is from the older ISO C
  // standard. Also see OS::snprintf() for more info on this.

  int result;
  va_list ap;
  va_start (ap, format);
  OSCALL (::vswprintf (buf, format, ap), int, -1, result);
  va_end (ap);
  return result;
