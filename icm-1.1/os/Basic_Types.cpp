...
ACE_RCSID (ace,
           Basic_Types,
           "$Id: Basic_Types.cpp,v 1.2 2009/03/19 05:29:23 qinghua Exp $")
...
{
  if (h_ () > 0)
    OS::fprintf (file, "0x%lx%0*lx", h_ (), 2 * sizeof l_ (), l_ ());
  else
    OS::fprintf (file, "0x%lx", l_ ());
}
