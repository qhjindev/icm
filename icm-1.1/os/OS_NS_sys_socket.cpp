// $Id: OS_NS_sys_socket.cpp,v 1.2 2009/03/19 05:29:23 qinghua Exp $
...
#include "os/os_include/netinet/os_in.h"
                ...
                int *addrlen)
{
  ...
  // EWOULDBLOCK.  Rather than hack the OSCALL_RETURN macro, it's
  // handled explicitly here.  If the OSCALL macro ever changes,
...
ACE_INLINE int
OS::bind (ACE_HANDLE handle, struct sockaddr *addr, int addrlen)
{
  OS_TRACE ("OS::bind");
...
ACE_INLINE int
OS::closesocket (ACE_HANDLE handle)
{
  OS_TRACE ("OS::closesocket");
...
ACE_INLINE int
OS::enum_protocols (int *protocols,
...
ACE_INLINE int
OS::getpeername (ACE_HANDLE handle, struct sockaddr *addr,
                     int *addrlen)
{
  OS_TRACE ("OS::getpeername");
  ...
  if (result != -1 && addr->sa_family == AF_INET)
    {
      OS::memset (reinterpret_cast<struct sockaddr_in *> (addr)->sin_zero,
                      0,
...
ACE_INLINE int
OS::getsockname (ACE_HANDLE handle,
                     struct sockaddr *addr,
                     int *addrlen)
{
  OS_TRACE ("OS::getsockname");
...
ACE_INLINE int
OS::getsockopt (ACE_HANDLE handle,
                    int level,
                    int optname,
                    char *optval,
                    int *optlen)
{
  OS_TRACE ("OS::getsockopt");
...
ACE_INLINE int
OS::listen (ACE_HANDLE handle, int backlog)
{
  OS_TRACE ("OS::listen");
...
ACE_INLINE ssize_t
OS::recv (ACE_HANDLE handle, char *buf, size_t len, int flags)
{
  OS_TRACE ("OS::recv");
...
ACE_INLINE ssize_t
OS::recvmsg (ACE_HANDLE handle, struct msghdr *msg, int flags)
{
  OS_TRACE ("OS::recvmsg");
...
ACE_INLINE ssize_t
OS::send (ACE_HANDLE handle, const char *buf, size_t len, int flags)
{
  OS_TRACE ("OS::send");
...
ACE_INLINE int
OS::setsockopt (ACE_HANDLE handle,
                    int level,
                    int optname,
                    const char *optval,
                    int optlen)
{
  OS_TRACE ("OS::setsockopt");
...
ACE_INLINE int
OS::shutdown (ACE_HANDLE handle, int how)
{
  OS_TRACE ("OS::shutdown");
...
ACE_INLINE int
OS::socketpair (int domain, int type,
                    int protocol, ACE_HANDLE sv[2])
{
  OS_TRACE ("OS::socketpair");
  ...
  ACE_NOTSUP_RETURN (-1);
#else
  OSCALL_RETURN (::socketpair (domain, type, protocol, sv),
                     int, -1);
...
OS::if_nametoindex (const char *ifname)
{
  OS_TRACE ("OS::if_nametoindex");
  OSCALL_RETURN (::if_nametoindex (ifname), int, 0);
}

ACE_INLINE char *
OS::if_indextoname (unsigned int ifindex, char *ifname)
{
  OS_TRACE ("OS::if_indextoname");
  OSCALL_RETURN (::if_indextoname (ifindex, ifname), char *, 0);
}

ACE_INLINE struct if_nameindex *
OS::if_nameindex (void)
{
  OS_TRACE ("OS::if_nameindex");
  OSCALL_RETURN (::if_nameindex (), struct if_nameindex *, 0);
}

ACE_INLINE void
OS::if_freenameindex (struct if_nameindex *ptr)
{
  OS_TRACE ("OS::if_freenameindex");
  if (ptr != 0)
...
ACE_HANDLE
OS::join_leaf (ACE_HANDLE socket,
                   const sockaddr *name,
                   int namelen,
                   const ACE_QoS_Params &qos_params)
{
...
#   if defined (ACE_HAS_WINCE)
        {
          ...
          OS::sprintf (buf, fmt, ACE_TEXT ("WSAStartup"), error);
          ::MessageBox (0, buf, ACE_TEXT ("WSAStartup failed!"), MB_OK);
        }
#   else
      OS::fprintf (stderr,
                       "OS::socket_init; WSAStartup failed, "
                         "WSAGetLastError returned %d\n",
                       error);
#   endif /* ACE_HAS_WINCE */

      OS::socket_initialized_ = 1;
    }
# else
...
#   if defined (ACE_HAS_WINCE)
          ACE_TCHAR fmt[] = ACE_TEXT ("%s failed, WSAGetLastError returned %d");
          ACE_TCHAR buf[80];  // @@ Eliminate magic number.
          OS::sprintf (buf, fmt, ACE_TEXT ("WSACleanup"), error);
          ::MessageBox (0, buf , ACE_TEXT ("WSACleanup failed!"), MB_OK);
#   else
          OS::fprintf (stderr,
                           "OS::socket_fini; WSACleanup failed, "
                             "WSAGetLastError returned %d\n",
                           error);
#   endif /* ACE_HAS_WINCE */
        }
      OS::socket_initialized_ = 0;
    }
# endif /* ACE_WIN32 */
  return 0;
}
