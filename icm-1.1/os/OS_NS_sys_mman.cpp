// $Id: OS_NS_sys_mman.cpp,v 1.2 2009/03/19 05:29:23 qinghua Exp $
...
ACE_INLINE int
OS::madvise (caddr_t addr, size_t len, int map_advice)
{
  OS_TRACE ("OS::madvise");
#if !defined (ACE_LACKS_MADVISE)
  OSCALL_RETURN (::madvise (addr, len, map_advice), int, -1);
#else
  ACE_UNUSED_ARG (addr);
  ACE_UNUSED_ARG (len);
  ...
  if (*file_mapping == ACE_INVALID_HANDLE)
    {
      ...
      const LPSECURITY_ATTRIBUTES attr =
        OS::default_win32_security_attributes_r (sa,
                                                     &sa_buffer,
                                                     &sd_buffer);
...
#  if defined (OS_EXTRA_MMAP_FLAGS)
  nt_flags |= OS_EXTRA_MMAP_FLAGS;
#  endif /* OS_EXTRA_MMAP_FLAGS */
...
#  if defined (OS_EXTRA_MMAP_FLAGS)
  flags |= OS_EXTRA_MMAP_FLAGS;
#  endif /* OS_EXTRA_MMAP_FLAGS */
  ACE_UNUSED_ARG (file_mapping);
#  if defined (ACE_OPENVMS)
  ::fsync(file_handle);
#  endif
  OSCALL_RETURN ((void *) ::mmap ((ACE_MMAP_TYPE) addr,
                                      len,
                                      prot,
                                      flags,
                                      file_handle,
...
ACE_INLINE int
OS::mprotect (void *addr, size_t len, int prot)
{
  OS_TRACE ("OS::mprotect");
#if defined (ACE_WIN32) && !defined (ACE_HAS_PHARLAP)
  DWORD dummy; // Sigh!
  return ::VirtualProtect(addr, len, prot, &dummy) ? 0 : -1;
#elif !defined (ACE_LACKS_MPROTECT)
  OSCALL_RETURN (::mprotect ((ACE_MMAP_TYPE) addr, len, prot), int, -1);
#else
  ACE_UNUSED_ARG (addr);
  ACE_UNUSED_ARG (len);
  ACE_UNUSED_ARG (prot);
...
ACE_INLINE int
OS::msync (void *addr, size_t len, int sync)
{
  OS_TRACE ("OS::msync");
#if defined (ACE_WIN32) && !defined (ACE_HAS_PHARLAP)
  ACE_UNUSED_ARG (sync);
...
# if !defined (ACE_HAS_BROKEN_NETBSD_MSYNC)
  OSCALL_RETURN (::msync ((ACE_MMAP_TYPE) addr, len, sync), int, -1);
# else
  OSCALL_RETURN (::msync ((ACE_MMAP_TYPE) addr, len), int, -1);
  ACE_UNUSED_ARG (sync);
...
ACE_INLINE int
OS::munmap (void *addr, size_t len)
{
  OS_TRACE ("OS::munmap");
...
#elif !defined (ACE_LACKS_MMAP)
  OSCALL_RETURN (::munmap ((ACE_MMAP_TYPE) addr, len), int, -1);
#else
  ACE_UNUSED_ARG (addr);
  ACE_UNUSED_ARG (len);
  ACE_NOTSUP_RETURN (-1);
...
OS::shm_open (const ACE_TCHAR *filename,
                  int mode,
                  mode_t perms,
                  LPSECURITY_ATTRIBUTES sa)
{
  OS_TRACE ("OS::shm_open");
  ...
  filename = buf;
#endif
  OSCALL_RETURN (::shm_open (ACE_TEXT_ALWAYS_CHAR(filename), mode, perms), ACE_HANDLE, ACE_INVALID_HANDLE);
#elif defined (ACE_OPENVMS)
  OSCALL_RETURN (::open (filename, mode, perms, ACE_TEXT("shr=get,put,upd")), ACE_HANDLE, ACE_INVALID_HANDLE);
#else  /* ! ACE_HAS_SHM_OPEN */
  // Just use ::open.
  return OS::open (filename, mode, perms, sa);
#endif /* ACE_HAS_SHM_OPEN */
}

ACE_INLINE int
OS::shm_unlink (const ACE_TCHAR *path)
{
  OS_TRACE ("OS::shm_unlink");
#if defined (ACE_HAS_SHM_OPEN)
#if defined (ACE_VXWORKS) && (ACE_VXWORKS <= 0x650)
  // With VxWorks the file should just start with / and no other
  // slashes, so replace all other / by _
  ...
  path = buf;
#endif
  OSCALL_RETURN (::shm_unlink (ACE_TEXT_ALWAYS_CHAR(path)), int, -1);
#else  /* ! ACE_HAS_SHM_OPEN */
  // Just use ::unlink.
