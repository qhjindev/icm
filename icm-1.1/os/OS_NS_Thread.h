 ...
 *  @file   OS_NS_Thread.h
 *
 *  $Id: OS_NS_Thread.h,v 1.2 2009/03/19 05:29:23 qinghua Exp $
 *
...
#ifndef OS_NS_THREAD_H
# define OS_NS_THREAD_H
...
#   else  /* ! ACE_HAS_TSS_EMULATION */
      typedef OS_thread_key_t ACE_thread_key_t;
...
typedef thread_key_t OS_thread_key_t;
// Application TSS key type (use this type except in TSS Emulation)
#   if defined (ACE_HAS_TSS_EMULATION)
      typedef u_int ACE_thread_key_t;
#   else  /* ! ACE_HAS_TSS_EMULATION */
...
typedef int ACE_hthread_t;
// Native TSS key type
typedef unsigned int OS_thread_key_t;
// Application TSS key type (use this type except in TSS Emulation)
#   if defined (ACE_HAS_TSS_EMULATION)
      typedef u_int ACE_thread_key_t;
#   else  /* ! ACE_HAS_TSS_EMULATION */
...
public:
  ...
  /// Initialize the object using calls to OS::thr_self().
  ACE_Thread_ID (void);
...
public:
  ...
  /// Shutdown TSS emulation.  For use only by OS::cleanup_tss ().
...
#   if defined (ACE_HAS_THREAD_SPECIFIC_STORAGE)
  ...
  static OS_thread_key_t native_tss_key_;
...
#   define ACE_KEY_INDEX(OBJ,KEY) \
  u_int OBJ; \
  OS::memcpy (&OBJ, &KEY, sizeof (u_int))
# else
...
# endif /* ACE_HAS_WCHAR */
  ...
  /**
   * If <timeout> == 0, calls <OS::mutex_lock(m)>.  Otherwise,
   * this method attempts to acquire a lock, but gives up if the lock
...
# if defined (ACE_HAS_THREAD_SPECIFIC_STORAGE)
  ...
  int thr_getspecific_native (OS_thread_key_t key,
                       void **data);
# endif /* ACE_HAS_THREAD_SPECIFIC_STORAGE */
...
#   if defined (ACE_HAS_THREAD_SPECIFIC_STORAGE)
  ...
  extern ACE_Export
  int thr_keycreate_native (OS_thread_key_t *key,
                     ACE_THR_C_DEST);
#   endif /* ACE_HAS_THREAD_SPECIFIC_STORAGE */
...
# if defined (ACE_HAS_THREAD_SPECIFIC_STORAGE)
  ...
  extern ACE_Export
  int thr_keyfree_native (OS_thread_key_t key);
# endif /* ACE_HAS_THREAD_SPECIFIC_STORAGE */
...
# if defined (ACE_HAS_THREAD_SPECIFIC_STORAGE)
  ...
  extern ACE_Export
  int thr_setspecific_native (OS_thread_key_t key,
                              void *data);
# endif /* ACE_HAS_THREAD_SPECIFIC_STORAGE */
...
{
  friend int OS::event_init(ACE_event_t*, int, int, int, const char*, void*,int);
  friend int OS::event_destroy(ACE_event_t*);
  friend int OS::event_wait(ACE_event_t*);
  friend int OS::event_timedwait(ACE_event_t*, TimeValue*, int);
  friend int OS::event_signal(ACE_event_t*);
  friend int OS::event_pulse(ACE_event_t*);
  friend int OS::event_reset(ACE_event_t*);
protected:
