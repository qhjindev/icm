 ...
 */
class ACE_Export PosixProactor : public ProactorImpl
{
  /**
...
public:
   ...
   */
  virtual int post_completion (PosixAsynchResult *result) = 0;
  ...
  virtual AsynchReadStreamResultImpl *create_asynch_read_stream_result (ACE_Handler &handler,
                                                                                ACE_HANDLE handle,
  ...
  virtual AsynchWriteStreamResultImpl *create_asynch_write_stream_result (ACE_Handler &handler,
                                                                                  ACE_HANDLE handle,
                                                                                  MessageBlock &message_block,
  ...
  virtual AsynchReadFileResultImpl *create_asynch_read_file_result (ACE_Handler &handler,
                                                                            ACE_HANDLE handle,
                                                                            MessageBlock &message_block,
                                                                            u_long bytes_to_read,
                                                                            const void *act,
  ...
  virtual AsynchWriteFileResultImpl *create_asynch_write_file_result (ACE_Handler &handler,
                                                                              ACE_HANDLE handle,
                                                                              MessageBlock &message_block,
                                                                              u_long bytes_to_write,
                                                                              const void *act,
  ...
  virtual AsynchResultImpl *create_asynch_timer (ACE_Handler &handler,
                                                       const void *act,
                                                       const TimeValue &tv,
                                                       ACE_HANDLE event = ACE_INVALID_HANDLE,
                                                       int priority = 0,
...
protected:
   ...
   * POSIX Proators.
   */
  void application_specific_code (PosixAsynchResult *asynch_result,
 ...
 * operations issued.
 */
class ACE_Export PosixAiocbProactor : public PosixProactor
{
  ...
  friend class PosixAiocbAsynchOperation;
...
public:
  ...
  PosixAiocbProactor (size_t nmaxop = ACE_AIO_DEFAULT_SIZE);

  virtual Proactor_Type  get_impl_type (void);

  /// Destructor.
  virtual ~PosixAiocbProactor (void);
  ...
  virtual int post_completion (PosixAsynchResult *result);
...
protected:
  ...
  /// and PosixSigProactor
  PosixAiocbProactor (size_t nmaxop,
  ...
  /// Op code now is saved in PosixAsynchResult
  virtual int start_aio (PosixAsynchResult *result);

  /// Start deferred AIO if necessary
  int start_deferred_aio();

  /// Cancel running or deferred AIO
  virtual int cancel_aiocb ( PosixAsynchResult * result );

  /// Extract the results of aio.
  PosixAsynchResult *find_completed_aio (int &error_status,
                                               int &return_status,
                                               size_t &index,
                                               size_t &count);

  /// Find free slot to store result and aiocb pointer
  virtual int allocate_aio_slot (PosixAsynchResult *result);
  ...
  int  putq_result (PosixAsynchResult *result);

  /// Get "post_completed" result from the internal queue
  PosixAsynchResult * getq_result (void);
  ...
  /// issued currently.
  aiocb **aiocb_list_;
  PosixAsynchResult **result_list_;
 ...
 ACE_HANDLE notify_pipe_read_handle_ ;
  ...
  /// number of PosixAsynchResult's waiting for start
  /// i.e. deferred AIOs
  ...
  /// Queue which keeps "post_completed" PosixAsynchResult's
  UnboundedQueue<PosixAsynchResult *> result_queue_;
};
 ...
 */
class ACE_Export PosixSigProactor : public PosixAiocbProactor
{
   ...
   */
  friend class PosixSigAsynchOperation;
...
public:
   ...
   */
  PosixSigProactor (size_t nmaxop = ACE_AIO_DEFAULT_SIZE);
   ...
   * use this Proactor.
   */
  PosixSigProactor (const sigset_t mask_set,
                          size_t nmaxop = ACE_AIO_DEFAULT_SIZE);

  /// Destructor.
  virtual ~PosixSigProactor (void);
  ...
  /// Post a result to the completion port of the Proactor.
  /// now it is implemented in base PosixAiocbProactor class
  ///virtual int post_completion (PosixAsynchResult *result);
 ...
 * called.
 */
class ACE_Export PosixAsynchTimer : public PosixAsynchResult
{

  /// The factory method for this class is with the POSIX_Proactor
  /// class.
  friend class PosixProactor;
  friend class PosixSigProactor;

protected:
  /// Constructor.
  PosixAsynchTimer (ACE_Handler &handler,
                          const void *act,
                          const TimeValue &tv,
                          ACE_HANDLE event = ACE_INVALID_HANDLE,
                          int priority = 0,
