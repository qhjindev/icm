...
static char *file = "ControlUpdateLog.log";
...
Simple_Tester::open (void)
{
  ...
  this->input_file_ = OS::open (file,
                                    O_CREAT | O_RDWR | FILE_FLAG_OVERLAPPED);
...
*/
  ...
  if (this->wf_.open (*this, this->input_file_) == -1)
...
Simple_Tester::handle_read_file (const AsynchReadFile::Result &result)
{
  ...
  MessageBlock& mb = result.message_block();
  u_long byteTransferred = result.bytes_transferred();
  mb.rdPtr ()[result.bytes_transferred ()] = '\0';
  ...
  if (result.success ())
    {
      ...
      //sprintf(str,"%s","\r\nwrite success\r\n");
      //result.message_block ().base (str, strlen(str) + 1);
      // Read successful: write this to the file.
      if (this->wf_.write (mb, byteTransferred, byteTransferred) == -1)
	    {
