...
void IceUtil::Options::checkArgs(const string& shortOpt,
...
void IceUtil::Options::addOpt(const string& shortOpt,
                              const string& longOpt,
                              ArgType at,
                              string dflt,
                              RepeatType rt)
...
vector<string> IceUtil::Options::split(const string& line)
{
    ...
    case ANSIQuote: {
      ...
      case '\\': {
        ...
        case '7': {
          ...
          unsigned short us = 0;
          string::size_type j;
          for (j = i; j < i + 3 && j < l.size() && octalDigits.find_first_of(c = l[j])
              != string::npos; ++j) {
            us = us * 8 + c - '0';
          }
          i = j - 1;
...
vector<string> IceUtil::Options::parse(const vector<string>& args)
{
  IceUtil::RecMutex::Lock sync(_m);
...
vector<string> IceUtil::Options::parse(int argc, const char* const argv[])
{
  vector<string> vec;
...
bool IceUtil::Options::isSet(const string& opt) const
{
  IceUtil::RecMutex::Lock sync(_m);
  ...
  return pos->second.repeat == NoRepeat ? _opts.find(opt) != _opts.end() : _ropts.find(opt)
      != _ropts.end();
}

string IceUtil::Options::optArg(const string& opt) const
{
  IceUtil::RecMutex::Lock sync(_m);
...
vector<string> IceUtil::Options::argVec(const string& opt) const
{
  IceUtil::RecMutex::Lock sync(_m);
...
void IceUtil::Options::addValidOpt(const string& opt,
                                   LengthType lt,
                                   ArgType at,
                                   const string& dflt,
                                   RepeatType rt)
...
IceUtil::Options::ValidOpts::iterator IceUtil::Options::checkOpt(const string& opt, LengthType lt)
{
  ValidOpts::iterator pos = _validOpts.find(opt);
  if (pos == _validOpts.end()) {
...
void IceUtil::Options::setOpt(const string& opt, const string& val, RepeatType rt)
{
  if (rt == Repeat) {
...
IceUtil::Options::ValidOpts::const_iterator IceUtil::Options::checkOptIsValid(const string& opt) const
{
...
IceUtil::Options::ValidOpts::const_iterator IceUtil::Options::checkOptHasArg(const string& opt) const
{
  ValidOpts::const_iterator pos = checkOptIsValid(opt);
