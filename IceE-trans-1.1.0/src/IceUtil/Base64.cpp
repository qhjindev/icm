...
string IceUtil::Base64::encode(const vector<unsigned char>& plainSeq)
{
  string retval;
...
vector<unsigned char> IceUtil::Base64::decode(const string& str)
{
  string newStr;
...
char IceUtil::Base64::encode(unsigned char uc)
{
  if (uc < 26) {
    return 'A' + uc;
  }
...
unsigned char IceUtil::Base64::decode(char c)
{
  if (c >= 'A' && c <= 'Z') {
    return c - 'A';
  }
...
bool IceUtil::Base64::isBase64(char c)
{
  if (c >= 'A' && c <= 'Z') {
    return true;
  }
