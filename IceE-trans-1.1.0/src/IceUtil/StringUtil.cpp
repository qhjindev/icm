...
static string toOctalString(unsigned int n)
{
  string s;
  s.resize(32);
...
//
static void encodeChar(string::value_type b, string& s, const string& special)
{
  switch (b)
  {
  case '\\': {
...
//
string IceUtil::escapeString(const string& s, const string& special)
{
  string::size_type i;
  for (i = 0; i < special.size(); ++i) {
    if (static_cast<unsigned char> (special[i]) < 32 || static_cast<unsigned char> (special[i])
        > 126) {
...
//
static char decodeChar(const string& s,
...
//
static void decodeString(const string& s,
                         string::size_type start,
                         string::size_type end,
                         string& sb)
{
...
//
bool IceUtil::unescapeString(const string& s,
                             string::size_type start,
                             string::size_type end,
                             string& result)
{
...
//
string::size_type IceUtil::checkQuote(const string& s, string::size_type start)
{
...
//
bool IceUtil::match(const string& s, const string& pat, bool matchPeriod)
{
  assert(!s.empty());
  assert(!pat.empty());
