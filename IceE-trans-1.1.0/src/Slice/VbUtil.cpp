...
static string lookupKwd(const string& name, int baseTypes, bool mangleCasts = false)
{
  ...
  //
  static const string keywordList[] = { "AddHandler", "AddressOf", "Alias", "And", "AndAlso",
      "Ansi", "As", "Assembly", "Auto", "Boolean", "ByRef", "ByVal", "Byte", "CBool", "CByte",
      "CChar", "CDate", "CDbl", "CDec", "CInt", "CLng", "CObj", "CShort", "CSng", "CStr", "CType",
      "Call", "Case", "Catch", "Char", "Class", "Const", "Date", "Decimal", "Declare", "Default",
      ...
      "Get", "GetType", "GoSub", "GoTo", "Handles", "If", "Implements", "Imports", "In",
      ...
      "Not", "NotInheritable", "NotOverridable", "Nothing", "Object", "On", "Option", "Optional",
      "Or", "OrElse", "Overloads", "Overridable", "Overrides", "ParamArray", "Preserve", "Private",
      "Property", "Protected", "Public", "REM", "RaiseEvent", "ReDim", "ReadOnly", "RemoveHandler",
  ...
  bool found = binary_search(&keywordList[0], &keywordList[sizeof(keywordList)
      / sizeof(*keywordList)], name, Slice::CICompare());
  if (found) {
    return "[" + name + "]";
  }
...
//
string Slice::VbGenerator::fixId(const string& name, int baseTypes, bool mangleCasts)
{
  if (name.empty()) {
    return name;
  }
...
string Slice::VbGenerator::typeToString(const TypePtr& type)
{
  if (!type) {
    return "";
  }

  static const char* builtinTable[] = { "Byte", "Boolean", "Short", "Integer", "Long", "Single",
...
bool Slice::VbGenerator::isValueType(const TypePtr& type)
{
  BuiltinPtr builtin = BuiltinPtr::dynamicCast(type);
  if (builtin) {
    switch (builtin->kind())
...
void Slice::VbGenerator::writeMarshalUnmarshalCode(Output &out,
                                                   const TypePtr& type,
                                                   const string& param,
                                                   bool marshal,
                                                   bool streamingAPI,
...
void Slice::VbGenerator::writeSequenceMarshalUnmarshalCode(Output& out,
  ...
  if (builtin) {
    ...
    case Builtin::KindObjectProxy: {
      if (marshal) {
        ...
        if (builtin->kind() == Builtin::KindObject) {
          ...
          out.inc();
          if (streamingAPI) {
            out << nl << stream << ".readObject(CType(New IceInternal.SequencePatcher(" << param
                << ", GetType(Ice.Object), ix__), Ice.ReadObjectCallback))";
          } else {
            out << nl << stream << ".readObject(New IceInternal.SequencePatcher(" << param
                << ", GetType(Ice.Object), ix__))";
          }
          out.dec();
          out << nl << "Next";
        } else {
    ...
    default: {
      ...
      if (marshal) {
        ...
        if (!isArray) {
          out << nl << param << " = New " << fixId(seq->scoped()) << '(' << stream << ".read"
              << marshalName << "Seq())";
        } else {
    ...
    if (marshal) {
      ...
      if (!streamingAPI) {
        if (type->isVariableLength()) {
          ...
          out << nl << stream << ".checkFixedSeq(szx__, "
...
string Slice::VbGenerator::toArrayAlloc(const string& decl, const string& sz)
{
  int count = 0;
...
void Slice::VbGenerator::validateMetaData(const UnitPtr& u)
{
...
bool Slice::VbGenerator::MetaDataVisitor::visitModuleStart(const ModulePtr& p)
{
...
void Slice::VbGenerator::MetaDataVisitor::visitModuleEnd(const ModulePtr&)
{
}

void Slice::VbGenerator::MetaDataVisitor::visitClassDecl(const ClassDeclPtr& p)
{
  validate(p);
}

bool Slice::VbGenerator::MetaDataVisitor::visitClassDefStart(const ClassDefPtr& p)
{
  validate(p);
  return true;
}

void Slice::VbGenerator::MetaDataVisitor::visitClassDefEnd(const ClassDefPtr&)
{
}

bool Slice::VbGenerator::MetaDataVisitor::visitExceptionStart(const ExceptionPtr& p)
{
  validate(p);
  return true;
}

void Slice::VbGenerator::MetaDataVisitor::visitExceptionEnd(const ExceptionPtr&)
{
}

bool Slice::VbGenerator::MetaDataVisitor::visitStructStart(const StructPtr& p)
{
  validate(p);
  return true;
}

void Slice::VbGenerator::MetaDataVisitor::visitStructEnd(const StructPtr&)
{
}

void Slice::VbGenerator::MetaDataVisitor::visitOperation(const OperationPtr& p)
{
  validate(p);
}

void Slice::VbGenerator::MetaDataVisitor::visitParamDecl(const ParamDeclPtr& p)
{
  validate(p);
}

void Slice::VbGenerator::MetaDataVisitor::visitDataMember(const DataMemberPtr& p)
{
  validate(p);
}

void Slice::VbGenerator::MetaDataVisitor::visitSequence(const SequencePtr& p)
{
  validate(p);
}

void Slice::VbGenerator::MetaDataVisitor::visitDictionary(const DictionaryPtr& p)
{
  validate(p);
}

void Slice::VbGenerator::MetaDataVisitor::visitEnum(const EnumPtr& p)
{
  validate(p);
}

void Slice::VbGenerator::MetaDataVisitor::visitConst(const ConstPtr& p)
{
  validate(p);
}

void Slice::VbGenerator::MetaDataVisitor::validate(const ContainedPtr& cont)
{
