...
Slice::Preprocessor::Preprocessor(const string& path, const string& fileName, const string& args) :
  _path(path), _fileName(fileName), _args(args), _cppHandle(0)
{
}
...
void Slice::Preprocessor::printMakefileDependencies(Language lang)
{
  if (!checkInputFile()) {
    return;
  }
...
bool Slice::Preprocessor::checkInputFile()
{
  ...
  if (suffix != ".idl") {
    cerr << _path << ": input files must end with `.idl'" << endl;
    return false;
  }
...
string Slice::Preprocessor::searchIceCpp()
{
#ifndef _WIN32
