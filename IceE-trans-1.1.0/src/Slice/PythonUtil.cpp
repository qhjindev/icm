...
static string lookupKwd(const string& name)
{
  ...
  //
  static const string keywordList[] = { "and", "assert", "break", "class", "continue", "def",
...
static string getDictLookup(const ContainedPtr& cont, const string& suffix = string())
{
...
void Slice::Python::CodeVisitor::visitModuleEnd(const ModulePtr& p)
{
...
bool Slice::Python::CodeVisitor::visitClassDefStart(const ClassDefPtr& p)
{
  ...
  if (!ops.empty()) {
    ...
    //
    _out << sp << nl << "#" << nl << "# Operation signatures." << nl << "#";
  ...
  if (!p->isLocal()) {
    ...
    for (oli = ops.begin(); oli != ops.end(); ++oli) {
      ...
      _out << ", _ctx=None):";
      _out.inc();
      _out << nl << "return _M_" << abs << "._op_" << (*oli)->name() << ".invoke(self, ("
          << inParams;
      ...
      if (p->hasMetaData("ami") || (*oli)->hasMetaData("ami")) {
        ...
        _out << ", _ctx=None):";
        _out.inc();
        _out << nl << "return _M_" << abs << "._op_" << (*oli)->name()
            << ".invokeAsync(self, _cb, (" << inParams;
        if (!inParams.empty() && inParams.find(',') == string::npos) {
          _out << ", ";
        }
        _out << "), _ctx)";
    ...
    _out.inc();
    _out << nl << "return _M_" << abs << "Prx.ice_checkedCast(proxy, '" << scoped
        << "', facetOrCtx, _ctx)";
    _out.dec();
    ...
    _out << sp << nl << "_M_" << type << "Prx = IcePy.defineProxy('" << scoped << "', " << name
        << "Prx)";
  }
...
void Slice::Python::CodeVisitor::visitEnum(const EnumPtr& p)
{
  ...
  // Emit the type information.
  //
  _out << sp << nl << "_M_" << getAbsolute(p, "_t_") << " = IcePy.defineEnum('" << scoped << "', "
      << name << ", (";
...
string Slice::Python::CodeVisitor::getSymbol(const ContainedPtr& p)
{
  //
...
void Slice::Python::CodeVisitor::writeDefaultValue(const TypePtr& p)
{
  BuiltinPtr builtin = BuiltinPtr::dynamicCast(p);
  if (builtin) {
    switch (builtin->kind())
...
void Slice::Python::CodeVisitor::collectClassMembers(const ClassDefPtr& p,
                                                     MemberInfoList& allMembers,
                                                     bool inherited)
{
  ClassList bases = p->bases();
...
void Slice::Python::CodeVisitor::collectExceptionMembers(const ExceptionPtr& p,
                                                         MemberInfoList& allMembers,
                                                         bool inherited)
{
  ExceptionPtr base = p->base();
...
static string changeInclude(const string& inc, const vector<string>& includePaths)
{
...
void Slice::Python::generate(const UnitPtr& un,
                             bool all,
                             bool checksum,
...
bool Slice::Python::splitString(const string& str, vector<string>& args, const string& delim)
{
...
string Slice::Python::getAbsolute(const ContainedPtr& cont, const string& suffix)
{
...
void Slice::Python::printHeader(IceUtil::Output& out)
{
...
void Slice::Python::MetaDataVisitor::visitModuleEnd(const ModulePtr&)
{
}
...
void Slice::Python::MetaDataVisitor::visitClassDefEnd(const ClassDefPtr&)
{
}
...
void Slice::Python::MetaDataVisitor::visitExceptionEnd(const ExceptionPtr&)
{
}
...
void Slice::Python::MetaDataVisitor::visitStructEnd(const StructPtr&)
{
}
...
void Slice::Python::MetaDataVisitor::visitParamDecl(const ParamDeclPtr& p)
{
  validate(p);
}
...
void Slice::Python::MetaDataVisitor::validate(const ContainedPtr& cont)
{
  DefinitionContextPtr dc = cont->definitionContext();
  assert(dc);
